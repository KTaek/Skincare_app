...
export default function Body2DView({ onMarked }: { onMarked: (region: BodyRegion) => void }) {
  const [wrapSize, setWrapSize] = useState({ width: 0, height: 0 });
  const [point, setPoint] = useState<BodyPoint | null>(null);
  ...
  const onPress = (e: { nativeEvent: { locationX: number; locationY: number } }) => {
    ...
    const region = classifyBodyRegion(vx, vy);
    if (!region) return;
