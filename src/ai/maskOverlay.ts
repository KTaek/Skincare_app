...
const MAX_SIDE = 768;
/**
 * 판정 단위(region)별 색 — app_re에서는 판정 단위를 색이 아니라 "부위 번호"로 구분하므로
 * (screens/ExamResultScreen.tsx의 RegionSymptomsCard) 모든 판정 단위가 같은 색이다. 배열 길이가
 * 1이면 아래 REGION_RGB[region % REGION_RGB.length]가 항상 0번(이 색)을 가리킨다 — 판정 단위가
 * 몇 개든 전부 이 붉은색(theme의 sev3) 하나로 칠해진다.
 */
export const REGION_COLORS = ['#EB5757'] as const;
